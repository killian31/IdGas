...
def write_submissions(model, test_data_filename, output_filename, apply_feat_eng=True, polynomial_degree=2,
                              include_group_interactions=True,
                              include_humidity_interactions=True,
                              feature_selection=False, k_features=20, use_model_features=True, use_embed=True, embedder_model=None):
    """
    ...
    if use_embed and (embedder_model is not None):
      embedder_model.eval()
      test_data_tensor = torch.tensor(df_test_processed.drop(columns=["ID"]).values.astype("float32"), dtype=torch.float32)
      with torch.no_grad():
          test_embeddings = embedder_model.embed(test_data_tensor).cpu().numpy()
      embs_df = pd.DataFrame(test_embeddings, columns=[f"embed_{i}" for i in range(test_embeddings.shape[1])], index=df_test_processed.index)
    ...
    is_selective_linear = hasattr(model, 'selected_features') and hasattr(model, 'feature_names')
    ...
    if is_selective_linear and use_model_features:
        ...
        available_features = [col for col in df_test_processed.columns if col in selected_feature_names or col == 'ID']
...
def predict_model(model, x):
    ...
    if hasattr(model, 'selected_features') and hasattr(model, 'feature_names'):
        ...
        if isinstance(X, pd.DataFrame):
            ...
            feature_mapping = {i: [(idx, idx) for idx in selected if idx < X_array.shape[1]] 
