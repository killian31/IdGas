...
def create_polynomial_features(df, degree=2, include_bias=False, interaction_only=False):
    """
...
def create_sensor_group_interactions(df):
    ...
    for group, prefix in [(m12_15_group, 'M12_15'), (m4_7_group, 'M4_7'), (rs_group, 'RS')]:
        if all(col in df.columns for col in group):
            ...
            df_result[f'{prefix}_range'] = df_result[f'{prefix}_max'] - df_result[f'{prefix}_min']
    
    # Create cross-group interactions
    if 'M12_15_mean' in df_result.columns and 'M4_7_mean' in df_result.columns:
        df_result['M12_15_M4_7_ratio'] = df_result['M12_15_mean'] / (df_result['M4_7_mean'] + 1e-8)
        df_result['M12_15_M4_7_product'] = df_result['M12_15_mean'] * df_result['M4_7_mean']
    
    if 'M12_15_mean' in df_result.columns and 'RS_mean' in df_result.columns:
        df_result['M12_15_RS_ratio'] = df_result['M12_15_mean'] / (df_result['RS_mean'] + 1e-8)
    ...
    if 'M4_7_mean' in df_result.columns and 'RS_mean' in df_result.columns:
        df_result['M4_7_RS_ratio'] = df_result['M4_7_mean'] / (df_result['RS_mean'] + 1e-8)
...
def create_humidity_interactions(df):
    ...
    for group_mean in ['M12_15_mean', 'M4_7_mean', 'RS_mean']:
        if group_mean in df_result.columns:
            df_result[f'Humidity_{group_mean}_product'] = df_result['Humidity'] * df_result[group_mean]
            df_result[f'Humidity_{group_mean}_ratio'] = df_result[group_mean] / (df_result['Humidity'] + 1e-8)
...
def apply_feature_engineering(df, target_df=None, polynomial_degree=2, 
                             include_group_interactions=True,
                             include_humidity_interactions=True,
                             feature_selection=True, k_features=20):
    """
    ...
    if polynomial_degree > 1:
        ...
        if sensor_cols:
            ...
            result_df = pd.merge(result_df, poly_df, on=['ID'] if 'ID' in df.columns else None)
    ...
    if feature_selection and target_df is not None:
        result_df = select_features(result_df, target_df, method='mutual_info', k=k_features)
